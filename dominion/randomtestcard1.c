...
int main(int argc, char** argv)
{
    ...
    int i, test, rand_seed, players, x, num_cards, new_num_cards, play,  passes, fails, totalpass, totalfail, num_buys, new_num_buys;

    struct gameState state;
    if(argc != 2)
    {
        printf("Usage: ./test_1 [rand seed]\n");
        return 0;
    }
    totalpass = 0;
    totalfail = 0;
    ...
    for(test = 0; test < MAX_TESTS; test++)
    {
        ...
        for(i = 0; i < players; i++)
        {
            ...
            {   
                ...
                new_num_buys = state.numBuys;

                if(new_num_cards - num_cards != 3 || new_num_buys != num_buys)
                {
                    fails++;
                }
                else
